...
export interface FileTreeProps {
  ...
  ariaLabel?: string;
  /**
   * Optional callback to resolve custom icons for files/directories.
   * Return a Lucide icon component or undefined to use default logic.
   * @param node - The file node to resolve an icon for
   * @returns Lucide icon component or undefined for default behavior
   */
  iconResolver?: (node: FileNode) => React.ComponentType<{ className?: string }> | undefined;
}
...
function TreeNode({
  ...
  // Try custom resolver first, fall back to default logic
  const customIcon = iconResolver?.(node);
  const IconComponent = customIcon ?? (
    isDirectory ? (isExpanded ? FolderOpen : Folder) : getFileIcon(node.name)
  );
 ...
 *   onSelect={(path) => setSelectedPath(path)}
 *   iconResolver={(node) => {
 *     if (node.path.includes('/agents/')) return Users;
 *     if (node.path.includes('/skills/')) return Zap;
